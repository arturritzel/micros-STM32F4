...
void t(void) {
	for (int i=0; i != 1500; i++);  // delay simbolico
}

void t2(void) {
	for (int i=0; i != 800000; i++);  // delay +0,5s
}
...
void ligax(int qtd){ // função para ligar x quantidade de leds

	GPIOB->ODR = 0; // reseta os leds

	switch(8-qtd){ // liga [qtd] leds
		  case 1: l(7);
		  case 2: l(6);
		  case 3: l(5);
		  case 4: l(4);
...
int main(void)
{
  ...
  int p1, p2, p3, total, ligando = 0;

  while(1){

	  t();

	  GPIOB->ODR=0; // reseta as saidas
	  ...
	  ligax(total); // liga os leds

	  if((GPIOC->IDR&=0x8)==0x8){
		  while((GPIOC->IDR&=0x8)==0x8); // trava ate soltar o botão

		  ligando++;
		  if(total != 0) ligax(ligando);
