...
int main(void)
{
	...
	while (1){
		...
		// 1 ciclo = 0,1s, então 1,4s = 14 ciclos
		for(int i = 0; i < 16; i++){
			if(i == 0){ desliga(5); liga(3); liga(8); } // liga verde 1 e vermelho 2, desliga vermelho 1
			if(i == 5) liga(4); // liga amarelo 1
			if(i == 7){ desliga(3); desliga(4); liga(5); } // desliga verde 1, amarelo 1, liga vermelho 1

			if(i == 8){desliga(8); liga(6);} // desliga vermelho 2, liga verde 2
			if(i == 13) liga(7); // liga amarelo 2
			if(i == 15){ desliga(6); desliga(7); liga(8); } // desliga verde 2, amarelo 2, liga vermelho 2
